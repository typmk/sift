(ns net.typemark.sift.callgraph
  ...
  intraprocedural pass in `net.typemark.sift.security` is limited to.
...
(def ^:private js-max-row
  ...
  compiles to ClojureScript too, the same reason `format` is banned here."
  1000000)
...
(def ^:private computed-dispatch
  ...
  string a tree-sitter parser uses, so the two name the same arm."
  "<computed>")

(defn- literal-dispatch?
  "Whether this `dispatch-val-str` is a value rather than a form to evaluate.

  A tree-sitter consumer decides this on the node type; kondo gives text only, so
  ...
  a tree-sitter consumer admits it too."
  [d]
  (and (seq d)
       (or (= \" (first d))
           (not (some #{\( \) \{ \} \~ \@ \` \#} d)))))
...
(defn- arm-name
  ...
  the node name a tree-sitter parser gives the same arm.

  A dispatch value that is a FORM names nothing, and taking its text produced
  `handle::(:k m)`, a node shaped like an arm that names no arm. That parser stopped
  ...
  a production service's analysis: 42 defmethod-marked usages, 40 literal and 2 not, both
  `(:flaky ops)`."
  [u]
...
(defn- owner-regions
  ...
  reported clean because it never saw the node. Measured on a code-graph tool's source:
  ...
  (let [impls (for [p (get a "protocol-impls" [])]
        ...
        ;; where the defmulti does -- which is also how a tree-sitter parser names it.
  ...
  (let [graph (call-graph analysis-text)
    ...
    (for [caller paths
      ...
      {:rule "interprocedural-taint"
       ...
       ;; compiles to ClojureScript too.
       ;; name the ends: the self-run on a code-graph tool gave 21 of these and not one
