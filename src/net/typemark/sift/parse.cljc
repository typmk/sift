...
(defn- line-starts
  "The offset of each line's first character in `s`."
  [s]
  (loop [from 0 acc [0]]
...
(defn nodes-of
  ...
  A leaf carries its :text. An inner node does not: `tree/text` slices it
  from the source this keeps on the stream's metadata, where storing it
  copied every form once for each form enclosing it."
  [root]
