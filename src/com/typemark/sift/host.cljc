(ns com.typemark.sift.host
  ...
  already says, with positions. defnet reads those as `ingest op=hostwarn`
  (DEFNET-51). These four are the ones the source states outright:
  ...
  - :js-prop-on-own-object  (.-k obj) where obj was built by #js/clj->js in
                         ...
                         mismatch defnet shipped (CLAUDE.md, extern trap).
...
(def rules
  {:catch-all-swallow     {:category :warning
