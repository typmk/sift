...
(deftest a-computed-dispatch-value-is-not-taken-as-a-name
  (let [g (cg/call-graph computed-arm)]
    ...
    (testing "the arm is kept under the name a tree-sitter parser gives it, so the two join"
...
(deftest a-vector-dispatch-value-is-still-a-name
  (let [g (cg/call-graph computed-arm)]
    (testing "isa? dispatch is a literal, and a tree-sitter consumer admits it too"
