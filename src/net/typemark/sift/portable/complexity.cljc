(ns net.typemark.sift.portable.complexity
  (:require [clojure.string :as str]))

(def ^:dynamic *ops* nil)

(defn- tag [x] (when x ((:tag *ops*) x)))
(defn- text [x] (when x ((:text *ops*) x)))
(defn- inner? [x] (boolean (and x ((:inner? *ops*) x))))
(defn- children [x] ((:children *ops*) x))
(defn- fn-params [x] ((:fn-params *ops*) x))
...
(defn- kids
  [node]
  (when (inner? node)
    (remove #(contains? trivia (tag %)) (children node))))
...
(defn- lst? [node] (contains? #{:list :fn} (tag node)))
(defn- vec? [node] (= :vector (tag node)))
...
(defn- string-node?
  [node]
  (and node (or (= :multi-line (tag node))
                (and (= :token (tag node)) (str/starts-with? (text node) "\"")))))

(defn- skip-doc-and-meta [nodes]
  (cond-> nodes
    (string-node? (first nodes)) rest
    true (as-> ns' (if (some-> (first ns') tag (= :map)) (rest ns') ns'))))
...
(defn- arities
  [rest-nodes]
  (if (vec? (unwrap-meta (or (first rest-nodes) nil)))
...
(defn- else-keyword? [node]
  (contains? #{":else" ":default"} (some-> node text)))
    ...
    (let [[tag payload] (kids node)
      ...
      (cond
        ...
        (= t "?")  (let [pairs (partition-all 2 (kids payload))
                         want  (:features ctx)]
                     (or (some (fn [[k v]]
                                 (when (contains? want (some-> k text))
                                   (lower ctx v)))
                               pairs)
                         []))
        :else []))
    (if (inner? node) (lower-seq ctx (kids node)) [])))
...
(defn report-forms
  [ops forms features]
  (binding [*ops* ops]
    (guarded #(let [ctx {:top? true :features features}
                    units (lower-seq ctx forms)]
                {:ok? true
...
(defn report-root
  [ops root features]
  (binding [*ops* ops]
    (report-forms ops (kids root) features)))
